...
def read_data_csv():
    ...
    with open(filenames['raw_data'], 'rb') as csvfile:
        ...
        times, nodes, entries1, entries2 =\
               list(map(list,list(zip(*[extract_data_from_line(line) for line in reader]))))
...
def write_data_coo(xxx_todo_changeme):
    (rows, columns_offset_by_one, trips, traveltimes) = xxx_todo_changeme
...
def find_full_links():
    ...
    potential_full_links = list(range(1, TOTAL_LINKS + 1+1))
...
def find_empty_links():
    ...
    reader = reader(open(filenames['data_trips'], 'rb'))
    print('data_trips.csv is opened')
    potential_empty_links = list(range(1, TOTAL_LINKS + 1+1))
...
def write_full_link_data():
    ...
    with open(filenames['data_trips'],'rb') as readfile:
        ...
        for line in reader:            
            V.append(list(map(float, [line[i-1] if bool(line[i-1]) else nan
...
def read_full_link_json():
    ...
    if TRIPS == 0:
        ...
        filename = filenames['full_link_speed']
        print 'Speed'
    else:
...
def write_data_array_transpose():
    ...
    hours = list(map(int, hours[5:].strip().split(',')))
    link_ids = list(map(int, link_ids[22:].strip().split(','))) # this list includes duplicates
    trips = trips[6:].strip().split(',')
    hours, link_ids, trips = list(zip(*[(h,l,t) for (h,l,t)
